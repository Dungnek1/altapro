...
const DeviceList = React.memo((props: DeviceListProps) => {
  ...
  const customPagination = {
    current: currentPageRef.current, // Current page number
      ...
      render: (text: string, record: any, index: number) => (
        <>
          ...
          <a
            href="#"
            onClick={() => {
              // setIsModelDeleteServiceOpen(true);
              setIsModelDeleteOpen(true);
              setDeletedDevice(record.deviceCode);
            }}
          >
            Xóa
          </a>
        ...
        <>
          ...
          <a
            ...
            onClick={() => {
              // setIsModelDeleteServiceOpen(true);
              setIsModelDeleteOpen(true);
              setDeletedService(record.serviceCode);
            }}
          >
            Xóa
          </a>
  ...
  const deleteDevice = async (device: string) => {
    return new Promise((resolve) => {
      fetch(process.env.REACT_APP_API_URL + "api/Device/" + device, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const deleteService = async (service: string) => {
    return new Promise((resolve) => {
      fetch(process.env.REACT_APP_API_URL + "api/Service/" + service, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      case 1:
        setDisplayData(
          filter1Value == "All"
            ...
            : internalData.filter((x: any) => x.operationStatus == filter1Value)
        );
        setFilter2Value(value);
        break;
      case 2:
  ...
  useEffect(() => {
    ...
    getPN();
  }, [currentPageRef.current, pageSize, isModalOpen]);

  const handleDelete = async () => {
    switch (props.columns) {
      case 1:
        let response: any = await deleteDevice(deletedDevice);
        if (response.ok) {
          setLoading(true);
          getPN();
          setLoading(false);
        ...
        break;
      case 2:
        let response1: any = await deleteService(deletedService);
        ...
        break;
      case 4:
        let response2: any = await deleteUser(deletedEmail);
        if (response2.ok) {
          setLoading(true);
          const [totalRecords, tempData] = await Promise.all([
            getTotalRecord(
    ...
    <div className="device-list">
      ...
      <div className="middleData">
        <Table
          ...
          loading={{ spinning: loading, delay: 50 }}
          pagination={
            props.columns == 1 || props.columns == 2
              ? { pageSize: 8 }
              : customPagination
      ...
      <Modal
        ...
        className="custom-modal"
      >
        <h3>Bạn muốn xóa thông tin này?</h3>
      </Modal>
    </div>
  );
});
